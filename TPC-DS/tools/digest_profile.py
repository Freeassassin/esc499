#!/usr/bin/env python3
"""Digest DuckDB JSON profiling output for TPC-DS into a Markdown report."""
...
def analyze_query(query_id: int, data: dict) -> dict:
    ...
    for col in join_key_cols:
        col_lower = col.lower()
        if col_lower.endswith(("key", "_sk", "_id")):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Digest TPC-DS DuckDB profiling output into a Markdown report")
    parser.add_argument("--scale", type=int, default=1)
    args = parser.parse_args()

    profile_dir = ROOT_DIR / "logs" / "duckdb" / f"sf{args.scale}" / "profile"
    ...
    lines: list[str] = []
    lines.append(f"# TPC-DS DuckDB Profiling Report (SF={args.scale})")
    lines.append("")
    total_latency = sum(r["latency"] for r in valid)
    total_cpu = sum(r["cpu_time"] for r in valid)
    ok_count = len(valid)
    err_count = len(rows) - ok_count
    lines.append(f"**Total latency**: {format_time(total_latency)} | **Total CPU time**: {format_time(total_cpu)} | **OK**: {ok_count} | **Missing**: {err_count}")
    lines.append("")
    ...
    lines.append("")
    lines.append("All TPC-DS queries are unique templates executed individually (no repetition within a single stream).")
    lines.append("")
