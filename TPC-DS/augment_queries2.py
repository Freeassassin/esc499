...
for path_str in all_files:
    ...
    if path_str in metadata_files:
        if is_ds:
            wrapper = f"[_LIMITA] SELECT * FROM information_schema.tables CROSS JOIN ( {bare_query.replace('[_LIMITA]', '')} ) subq [_LIMITC];"
        else:
            wrapper = f"SELECT * FROM information_schema.tables CROSS JOIN ( {bare_query} ) subq;"
    elif path_str in maint_files:
        if is_ds:
            wrapper = f"[_LIMITA] SELECT 1 as maintain_dummy FROM ( {bare_query.replace('[_LIMITA]', '')} ) subq [_LIMITC];"
        else:
            wrapper = f"SELECT 1 as maintain_dummy FROM ( {bare_query} ) subq;"
    elif path_str in update_files:
        if is_ds:
            wrapper = f"UPDATE customer SET c_birth_year = (SELECT count(*) FROM ( {bare_query.replace('[_LIMITA]', '')} ) subq) WHERE c_customer_sk = -1;\n"
        else:
        ...
        if is_ds:
            wrapper = f"DELETE FROM customer WHERE c_customer_sk = (SELECT count(*) FROM ( {bare_query.replace('[_LIMITA]', '')} ) subq) AND c_customer_sk = -1;\n"
        else:
        ...
        if is_ds:
            wrapper = f"INSERT INTO customer (c_customer_sk) SELECT count(*) FROM ( {bare_query.replace('[_LIMITA]', '')} ) subq WHERE 1=0;\n"
        else:
            wrapper = f"INSERT INTO region (r_regionkey) SELECT count(*) FROM ( {bare_query} ) subq WHERE 1=0;\n"
    elif path_str in ctas_files:
        if is_ds:
            wrapper = f"[_LIMITA] CREATE TEMP TABLE t_temp_{random.randint(1,999)} AS SELECT * FROM ( {bare_query.replace('[_LIMITA]', '')} ) subq [_LIMITC];\n"
        else:
