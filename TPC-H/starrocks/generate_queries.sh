...
for i in $(seq 1 22); do
  ...
  LC_ALL=C perl -0777 -i -pe 's/substring\(([^\)]+?) from ([0-9]+) for ([0-9]+)\)/substring($1, $2, $3)/gi' "${out_file}"
  LC_ALL=C perl -0777 -i -pe 's/extract\(year from ([^\)]+)\)/year($1)/gi' "${out_file}"

  rm -f "${raw_file}"
  echo "Generated ${out_file}"
done

echo "Generated all 22 StarRocks-compatible query files in ${OUT_DIR}."
